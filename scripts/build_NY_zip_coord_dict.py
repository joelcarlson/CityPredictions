...
if __name__=="__main__":
    ...
    #Create a dictionary with zipcode keys and (lat, long) items
    geolocator = Nominatim()
    zip_coords_dict = dict()

    for idx, zipcode in enumerate(NY_zips):
        if idx % 10 == 0:
            print "Processed {}/{} zipcodes.".format(idx, len(NY_zips))
        zip_coords_dict[zipcode] = geolocator.geocode(zipcode)[1]


    with open('data/NY_Info/zip_coords_dict.pickle', 'wb') as handle:
      pickle.dump(zip_coords_dict, handle)
